...
;(function($){
    ...
    $.aliasGenerator = {
        ...
        'init' : function($el, params) {
            ...
            var sourceJQEls = $(params['from']);
            var aliasJQEls = $(params['to']);
            if (!sourceJQEls.length || sourceJQEls.length != aliasJQEls.length || sourceJQEls.length != $el.length) {
                return false;
            }

            var sourceElements = this.sourceElements;
            var aliasElements = this.aliasElements;
            var buttonElements = this.buttonElements;

            $.each(sourceJQEls, function(id, srcEl) {
                sourceElements.push(srcEl);
                aliasElements.push(aliasJQEls[id]);
                buttonElements.push($el[id]);
            });
            this.sourceElements = sourceElements;
            this.aliasElements  = aliasElements;
            this.buttonElements = buttonElements;

            if ($el.is('INPUT') && $el.attr('type') != 'submit') {
                sourceInput.change(this.btnClickHandler);
            } else {
                $el.click(this.btnClickHandler);
            }
            // } catch (e) {}
        ...
        'btnClickHandler' : function () {
            try {
                ...
                var currentId = null;
                var sourceElements = $.aliasGenerator.sourceElements;
                var aliasElements  = $.aliasGenerator.aliasElements;
                var buttonElements = $.aliasGenerator.buttonElements;
                for (var id in buttonElements) {
                    if (buttonElements[id] == this) {
                        currentId = id;
                    }
                }
                ...
                // Если алиас уже заполнен, то не меняем его автоматически при изменении текста в источнике
                // if (this.tagName == 'INPUT' && $(this).attr('type') != 'submit' && $.aliasGenerator.aliasInput.val().trim()) {
                //     return false;
                // }

                var sourceVal = $(sourceElements[currentId]).val().trim().replace(/\s{2,}/g, ' ').toLowerCase();
                if (!sourceVal) {
                    return false;
                }
                var alias = $.aliasGenerator.translit(sourceVal).replace(/\s/g, '-').replace(/[^a-z0-9\-]/g, '');
                ...
                //     .done(function (data) {
                //         if (data.success) {
                //             $.aliasGenerator.aliasInput.val(data.alias);
                //             return false;
                //         }
                //     });
                ...
                $(aliasElements[currentId]).val(alias);

            } catch (e) {}

            return false;
        },
        'translit' : function translit(str, typ) {
            if (!typ) {
                typ = 5;
            }
    ...
    $.fn.aliasGenerator = function(params) {
        if (typeof params === 'object') {
            return $.aliasGenerator.init(this, params);
        }
...
// });
// $('.js-alias-generate-alias').aliasGenerator({
//     'from' : '.js-alias-generate-source',
//     'to' : '.js-alias-generate-alias'
// });
